...
{
    public static IEnumerable<Lazy<ControllerContext>> GetControllers(this Assembly assembly)
    {
        ...
        foreach (var assemblyName in assemblyNames)
        {
            ...
            foreach (var exportedType in exportedTypes)
            {
                if (exportedType.TryGetCustomAttribute<ControllerAttribute>(out ControllerAttribute controllerAttribute))
                {
                    ...
                    yield return new Lazy<ControllerContext>(() => new ControllerContext(exportedType, methodInfos));
                }
            }
        }
    }
